...
import {
  ...
  Activity, BarChart3, LogOut, User, TrendingUp,
} from 'lucide-react'
import { clsx } from 'clsx'

const navItems = (isHrd) => [
    ...
    <aside className="w-60 min-h-screen bg-white border-r border-slate-100 flex flex-col shrink-0">
      ...
      <div className="px-5 py-6 border-b border-slate-100">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-sapphire flex items-center justify-center">
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5">
        {navItems(isHrd).map(({ to, label, Icon }) => (
          <NavLink
            key={to}
            to={to}
            end={to === '/'}
      ...
      <div className="px-3 py-4 border-t border-slate-100">
        <div className="flex items-center gap-3 px-3 py-2 mb-1">
          <div className="w-8 h-8 rounded-full bg-ice-blue flex items-center justify-center">
            <span className="text-sapphire font-bold text-sm">
