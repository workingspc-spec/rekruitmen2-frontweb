...
export default function ApprovalListPage({ initialPeriodFilter = null }) {
    ...
    // ── Hitung count dari rawList LENGKAP (bukan subset) ─────────────────────
    // ✅ FIX: list sekarang selalu berisi semua item, sehingga count di sini akurat
    const pendingCount  = list.filter(item =>  isPending(item)).length
    const approvedCount = list.filter(item => !isPending(item)).length
    const allCount      = list.length

    // ── Filter gabungan: status + search + period ─────────────────────────────
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="flex flex-wrap gap-3 items-center">
                {/* Status Tabs dengan count badge */}
                <div className="flex gap-1 bg-slate-100 p-1 rounded-xl">
                    {STATUS_TABS.map(t => {
                        ...
                        return (
                            ...
                            >
                                {t.label}
                                {/* Count badge — hanya tampil saat count > 0 */}
                                {count > 0 && (
                                ...
                                {/* Dot merah: ada pending tapi tab ini tidak aktif */}
